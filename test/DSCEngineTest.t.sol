...
import "./mocks/MockMoreDebtDSC.sol";
...
contract DSCEngineTest is Test {
    ...
    function testGetTokenAmountFromUsd() public view {
        uint256 usdAmountInWei = 100 ether; //100usd
        uint256 ethAmountFromUsd = engine.getTokenAmountFromUsd(weth, usdAmountInWei);
        //1eth->2000USD
        uint256 expectResult = 0.05 ether;
        assertEq(expectResult, ethAmountFromUsd);
    }
    ...
    function testRevertIfCollateralNotSupported(address randomToken) public {
        ...
        engine.depositCollateral(randomToken, AMOUNT_COLLATERAL);
        vm.stopPrank();
    }
    ...
    function testCanDepositCollateralAndGetAccountInfo() public depositCollateral {
        ...
        uint256 expectedDepositAmountInUsd = engine.getTokenAmountFromUsd(weth, collateralValueInUsd);
        assertEq(expectedDepositAmountInUsd, AMOUNT_COLLATERAL);
    }
    ...
    function testCanRedeemCollateral() public depositCollateral {
        vm.startPrank(USER);
        uint256 preUserBalance = ERC20Mock(weth).balanceOf(USER);
        engine.redeemCollateral(weth, AMOUNT_COLLATERAL);
        uint256 userBalance = ERC20Mock(weth).balanceOf(USER);
        assertEq(userBalance, preUserBalance + AMOUNT_COLLATERAL);
        vm.stopPrank();
    }
